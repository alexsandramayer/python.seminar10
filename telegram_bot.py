...
from telegram.ext import CommandHandler, ContextTypes, ConversationHandler, Updater, MessageHandler, filters
...
def choice(update, context):
    ...
    log.info("Выбор операции: %s: %s", user.first_name, update.message.text)
...
def rational_one(update, context):
    user = update.message.from_user
    log.info("Пользователь ввел число: %s: %s", user.first_name, update.message.text)
...
def cancel(update, _):
    user = update.message.from_user
    log.info("Пользователь %s отменил разговор.", user.first_name)
    update.message.reply_text('Спасибо, до свидания!')
    return ConversationHandler.END
